...
import { env } from "../../env";
import { getTelegramMcpTools } from "./tool";
import type { ToolConfig } from "@iqai/adk";
import {
  TranscribeVoiceTool,
  TextToSpeechTool,
  TranslateToYorubaTool
} from "./voice-and-translation-tools";
...
export const getTelegramAgent = async (samplingHandler: SamplingHandler) => {
  ...
  const telegramAgent = new LlmAgent({
    name: "telegram_agent",
    description:
      "An agent capable of interacting with Telegram, handling voice messages and translations.",
    tools: [
      ...telegramMcpTools,
      new TranscribeVoiceTool({} as ToolConfig),
      new TextToSpeechTool({} as ToolConfig),
      new TranslateToYorubaTool({} as ToolConfig)
    ],
	model: env.LLM_MODEL,
  });
  return telegramAgent;
